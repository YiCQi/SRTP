...
projects=$(cat "QC项目列表路径")

for project in $projects; do
    project_folder="$input_dir/$project"

    output_dir="$project_folder/03_QC"
    mkdir -p "$output_dir"

    for sample in "$project_folder"/02_fastq_gz/*RR*_1*; do #只取_1文件防止QC两次
        sample_ID=$(basename "$sample" | sed -n 's/\(.*RR[0-9]*\).*/\1/p') #提取SRR*的ID部分
        ...
        if [ -e "$project_folder/02_fastq_gz/${sample_ID}_2.fastq.gz" ]; then
            ...
            "$output_dir/${sample_ID}_1_paired.fastq.gz" \
            "$output_dir/${sample_ID}_1_unpaired.fastq.gz" \
            "$output_dir/${sample_ID}_2_paired.fastq.gz" \
            "$output_dir/${sample_ID}_2_unpaired.fastq.gz" \
            ...
            trimmomatic SE \
            "$project_folder/02_fastq_gz/${sample_ID}_1.fastq.gz" \
            "$output_dir/${sample_ID}_trimmed.fastq.gz" \
            ILLUMINACLIP:Trimmomatic-adapter.fa:2:30:10 \
            LEADING:3 \
            TRAILING:3 \
            SLIDINGWINDOW:4:15 \
...
脚本本身没有问题。双端测序和单端测序也可以分别分析。

但是QC输出文件跑不通mixcr脚本，只生成vdjca文件。目前存疑。

猜测可能情况是阈值选择不正确，例如最小长度设置不合理，质量分数控制阈值不合理

mixcr的参数选择不正确可能会导致错误，但目前出现的跑不全问题一定是QC文件有问题导致的
...
projects=$(cat "mixcr项目列表路径")

for project in $projects; do
    project_folder="$input_dir/$project"

    output_dir="$project_folder/04_MixCR"
    mkdir -p "$output_dir"

    for sample in "$project_folder"/03_QC/*RR*_1_paired*; do
        echo $sample
        sample_ID=$(basename "$sample" | sed -n 's/\(.*RR[0-9]*\).*/\1/p')

        # 检查是否双端测序
        if [ -e "$project_folder/03_QC/${sample_ID}_2_paired.fastq.gz" ]; then
            ...
            mixcr analyze generic-tcr-amplicon \
                ...
                "${project_folder}/03_QC/${sample_ID}_1_paired.fastq.gz" \
                "${project_folder}/03_QC/${sample_ID}_2_paired.fastq.gz" \
            ...
            mixcr analyze generic-tcr-amplicon \
                ...
                --rigid-right-alignment-boundary \
                --keep-non-CDR3-alignments \
                "${project_folder}/03_QC/${sample_ID}_trimmed.fastq.gz" \
                "$output_dir/$sample_ID"/"$sample_ID" 
        fi
    done
done
...
即Immunoglobulin BLAST，用于序列比对，功能注释，突变分析，克隆分析。可以说与mixcr功能**很像**。

**不同之处：**

IgBLAST使用BLAST搜索算法。

IgBLAST使用NCBI提供的免疫球蛋白基因库作为参考序列库，而mixcr用自定义的参考序列库。

IgBLAST使用特定的替代突变模型来处理抗体和TCR序列中的突变，而MiXCR使用独特的测序错误模型来处理突变。

IgBLAST支持FASTA格式的序列作为输入。MiXCR支持FASTQ和FASTA格式的序列，并可以直接从测序数据中读取序列和质量信息。
